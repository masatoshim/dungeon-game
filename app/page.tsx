      ...
      <div className="max-w-4xl mx-auto">
          ...
          {dungeons.map((dungeon) => (
            ...
            >
              <div className="flex justify-between items-start mb-4">
                <h2 className="text-2xl font-bold group-hover:text-yellow-400 transition-colors">{dungeon.name}</h2>
                <span className="px-2 py-1 bg-gray-700 rounded text-xs">Lv.{dungeon.difficulty}</span>
              </div>
              <p className="text-gray-400 text-sm mb-4 line-clamp-2">
                {dungeon.description || "未知のダンジョンがあなたを待っている..."}
              </p>
              <div className="flex justify-between items-center text-sm font-mono text-gray-500">
                <span>Time: {dungeon.timeLimit}s</span>
                <span className="text-yellow-600 group-hover:text-yellow-400 font-bold">PLAY START →</span>
              </div>
            </Link>
          ))}
        </div>
        ...
        {dungeons.length === 0 && (
          <div className="text-center py-20 bg-gray-800 rounded-xl border border-dashed border-gray-600">
            ...
            <p className="text-sm text-gray-600">Prisma Studioなどでデータを追加してください。</p>
          </div>
        )}
      </div>
    </main>
